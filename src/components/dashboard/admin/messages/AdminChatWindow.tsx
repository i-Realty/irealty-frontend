...
function MessageBubble({ message }: { message: AdminMessage }) {
  ...
  const bubbleStyle = isAdmin
    ...
    : 'bg-white border border-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.03)] text-gray-700 rounded-r-2xl rounded-tl-2xl';
    ...
    <div className="flex flex-col h-full bg-white relative">
      ...
      <div className="flex-none px-4 py-3 bg-white border-b border-gray-100 flex items-center justify-between shadow-sm z-10 sticky top-0">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            <div className="relative w-9 h-9 rounded-full overflow-hidden bg-gray-200 border border-gray-100 flex-shrink-0">
              <Image
            ...
            <div className="flex flex-col min-w-0">
              <h2 className="text-[14px] font-bold text-gray-900 leading-tight truncate">
                {thread.user.name}
              </h2>
              <p className="text-[11px] text-gray-400 font-medium truncate max-w-[200px] md:max-w-[300px]">
                {thread.subject}
              </p>
            </div>
          </div>
      ...
      {/* Ticket meta strip */}
      <div className="flex items-center gap-3 px-4 py-2 bg-gray-50/50 border-b border-gray-100 text-[11px] font-medium text-gray-500">
        <span>Ticket: <span className="text-gray-700 font-bold">{thread.id.toUpperCase()}</span></span>
        <span className="w-px h-3 bg-gray-200" />
        <span>Category: <span className="text-gray-700 font-bold">{thread.category}</span></span>
        <span className="w-px h-3 bg-gray-200" />
        <span>Priority: <span className="text-gray-700 font-bold">{thread.priority}</span></span>
        <span className="w-px h-3 bg-gray-200" />
        <span>Created: <span className="text-gray-700">{thread.createdAt}</span></span>
      </div>

      {/* Messages */}
      <div className="flex-1 overflow-y-auto no-scrollbar p-4 md:p-6 bg-white pb-28">
      ...
      {thread.status !== 'Resolved' && (
        <div className="absolute bottom-0 w-full left-0 bg-white border-t border-gray-100 px-4 py-3">
          <div className="flex items-end gap-3">
            <textarea
              ...
              rows={1}
              className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-[14px] text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-colors resize-none min-h-[44px] max-h-[120px]"
              disabled={isSending}
            />
            <button
              onClick={handleSend}
