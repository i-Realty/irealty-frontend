...
export default function ProfileSettings() {
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Blob preview stays — will sync on next profile save
    } finally {
  ...
  return (
    <div className="flex flex-col gap-8 w-full animate-in slide-in-from-right-4 fade-in duration-300">
       ...
       <div className="flex flex-col">
          <h2 className="text-[18px] font-bold text-gray-900 tracking-tight mb-1">Profile information</h2>
          <p className="text-[13px] font-medium text-gray-400">Please provide the necessary personal information</p>
       </div>
           ...
           <div className="flex items-center gap-4 mb-8">
               ...
               <div className="flex flex-col">
                  <span className="text-[18px] font-bold text-gray-900">
                    {profile.displayName || `${profile.firstName} ${profile.lastName}`.trim() || user?.displayName || user?.name || 'User'}
                  </span>
