...
export default function Hero() {
  ...
  return (
    <section className="relative mt-20 flex flex-col items-center justify-center min-h-[600px] md:min-h-[700px] w-full overflow-hidden pt-8 pb-12 md:pt-16 md:pb-24">
      ...
      <div className="relative z-20 flex flex-col items-center justify-center w-full max-w-3xl mx-auto px-4 md:px-0">
        ...
        <div className="mt-6 flex flex-col items-center gap-4">
          <div className="flex items-center gap-4">
            ...
            <p className="text-white text-sm md:text-base text-center">{t('hero.cta')}</p>
            <div className="hidden sm:block w-28 h-px bg-white/40" />
          </div>

          <div className="flex gap-4 mt-2">
            <Link href="/auth/signup?role=property-owner" className="bg-[#2563EB] hover:bg-[#1e4fcf] transition-colors text-white font-bold rounded-lg px-6 py-3 text-sm md:text-base shadow-lg">{t('hero.postAds')}</Link>
            <Link href="/auth/signup?role=real-estate-agent" className="bg-transparent border border-white/40 text-white font-bold rounded-lg px-6 py-3 text-sm md:text-base hover:bg-white/10 transition-colors">{t('hero.becomeAgent')}</Link>
          </div>
        </div>
      </div>
    </section>
