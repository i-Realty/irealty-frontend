      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white border border-gray-100 rounded-xl p-6 shadow-sm">
            <h3 className="text-lg font-bold text-gray-900 mb-5">Financial Breakdown</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: 'Total Amount', value: `₦${tx.amount.toLocaleString()}`, color: 'text-gray-900' },
                ...
                { label: 'Net to Parties', value: `₦${tx.netToParties.toLocaleString()}`, color: 'text-gray-900' },
              ].map((item) => (
              ...
              {tx.auditLog.map((entry, i) => (
                <div key={i} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    ...
                    {i < tx.auditLog.length - 1 && <div className="w-0.5 flex-1 bg-gray-200 min-h-[16px]" />}
                  </div>
                  <div className="pb-4">
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white border border-gray-100 rounded-xl p-5 shadow-sm flex flex-col items-center text-center">
            ...
            <Image src={tx.partyAAvatar} alt={tx.partyA} width={48} height={48} className="w-12 h-12 rounded-full object-cover border-2 border-gray-100 mb-2" />
            <p className="text-sm font-bold text-gray-900 mb-3">{tx.partyA}</p>
            <button className="w-full flex items-center justify-center gap-2 border border-gray-200 rounded-lg py-2 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">
          ...
          <div className="bg-white border border-gray-100 rounded-xl p-5 shadow-sm flex flex-col items-center text-center">
            ...
            <Image src={tx.partyBAvatar} alt={tx.partyB} width={48} height={48} className="w-12 h-12 rounded-full object-cover border-2 border-gray-100 mb-2" />
            <p className="text-sm font-bold text-gray-900 mb-3">{tx.partyB}</p>
            <button className="w-full flex items-center justify-center gap-2 border border-gray-200 rounded-lg py-2 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">
              <MessageCircle className="w-4 h-4" /> Message
            </button>
          </div>
